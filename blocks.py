...
class Blocks_world():
    ...
    def execute_action(self,action):
        ...
        if tower.contains(block):
            if block.clear:
                popped = tower.pop_block()
                self.add_to_table(popped)
    ...
    def get_state_facts(self):
        ...
        for tower in self.towers:
            ...
            if not N:
                continue
            facts.append("on_table(s"+str(self.state_number)+","+str(blocks[0])+",table)")
            facts.append("clear(s"+str(self.state_number)+","+str(blocks[N-1])+")")
            for i in range(1,N):
                facts.append("on(s"+str(self.state_number)+","+str(blocks[i])+","+str(blocks[i-1])+")")
        table_blocks = self.get_table_blocks()
        for block in table_blocks:
            facts.append("on_table(s"+str(self.state_number)+","+str(block)+",table)")
            facts.append("clear(s"+str(self.state_number)+","+str(block)+")")
            '''
            if tower.too_high():
